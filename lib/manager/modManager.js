...
module.exports = function(config){
	...
	const instances = require("lib/getInstances")(config).sync();
	return {
		/**
		...
		@example
		var modManager = require("modManager")(config);
		...
		*/
		listMods: function(instance){
			return new Promise((resolve, reject) => {
				if(instance && instances.includes(instance)){
					factorioAPI.init(false, config.instanceDirectory+'/'+instance+'/instanceMods', config.instanceDirectory+'/'+instance+'/saves', config.factorio_version);
					...
					factorioAPI.init(false, 'sharedMods/', '', config.factorio_version);
					factorioAPI.authenticate({
						username: config.username,
						token: config.token,
						require_ownership: false
		...
		*/
		...
		removeMod: function removeMod(term, instance, force = false){
			return new Promise((resolve, reject) => {
				if(instance == "shared"){ // modDirectory has to end in /
					var modDirectory = "sharedMods/";
				} else {
					...
					if(modsMatchingFilter.length < 1){
						...
						if(query("Delete all mods matching filter?")){
							...
							for(let i = 0; i < modsToDelete; i++){
								fs.unlinkSync(modDirectory+modsMatchingFilter[i]);
		...
		*/
		downloadFromGitURL: function(url){
